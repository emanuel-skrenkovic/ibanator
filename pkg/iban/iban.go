...
// Really should not be in code, rather in a database as things
// change. Fine for a small example.
// Might be wrong, but it seems that Go returns references of maps
// instead of copies.
func getCountryIBANLength(cc string) map[string]int {
...
func validateLength(cc string, iban string) error {
	countryLengths := getCountryIBANLength(cc)
	requiredLength, found := countryLengths[cc]
	if !found {
